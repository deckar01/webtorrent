...
module.exports = class WebTorrent extends EventEmitter {
  /**
   * WebTorrent Client
   * @param {Object=} opts
   */
  constructor (opts) {
    ...
    if (typeof TCPPool === 'function') {
      ...
      process.nextTick(() => this._onListening())
    }
   ...
   */
  get (torrentId) {
    const len = this.torrents.length

    if (torrentId instanceof Torrent) {
      for (let i = 0; i < len; i++) {
        const torrent = this.torrents[i]
  ...
  add (torrentId, opts, ontorrent) {
    ...
    const onInfoHash = () => this._onInfoHash(torrent)
    const onTorrentReady = () => this._onTorrentReady(torrent, ontorrent)
    ...
    torrent.once('close', () => {
      ...
      torrent.removeListener('ready', onTorrentReady)
    })
  ...
  _onInfoHash (torrent) {
    if (this.destroyed) return
    for (let i = 0, len = this.torrents.length; i < len; i++) {
  ...
  _onTorrentReady (torrent, ontorrent) {
    if (this.destroyed) return
  ...
  seed (input, opts, onseed) {
    if (this.destroyed) throw new Error('client is destroyed')
    if (typeof opts === 'function') return this.seed(input, null, opts)
    ...
    let streams = null
    const torrent = this.add(null, opts, (torrent) => this._onTorrent(torrent, streams, onseed))

    if (isFileList(input)) input = Array.prototype.slice.call(input)
    if (!Array.isArray(input)) input = [ input ]

    parallel(input.map((item) => {
      return (cb) => {
  ...
  _onTorrent (torrent, streams, onseed) {
    const tasks = [(cb) => torrent.load(streams, cb)]
    if (this.dht) tasks.push((cb) => torrent.once('dhtAnnounce', cb))
    parallel(tasks, (err) => {
      if (this.destroyed) return
      if (err) return torrent._destroy(err)
      this._onseed(torrent, onseed)
    })
  }

  _onseed (torrent, onseed) {
    debug('on seed')
...
function isFileList (obj) {
  ...
  const supportsFileList = inBrowser && typeof window.FileList !== 'undefined'
  return supportsFileList && obj instanceof window.FileList
}
