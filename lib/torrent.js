...
let _TMP = null
try {
  _TMP = path.join(fs.statSync('/tmp') && '/tmp', 'webtorrent')
} catch (err) {
  _TMP = path.join(typeof os.tmpDir === 'function' ? os.tmpDir() : '/', 'webtorrent')
}
const TMP = _TMP
...
module.exports = class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    const noUploads = (opts.uploads === false || opts.uploads === 0)
    this._rechokeNumSlots = noUploads ? 0 : (+opts.uploads || 10)
  ...
  // get numMissing () {
  //   let numMissing = this.pieces.length
  //   for (let index = 0, len = this.pieces.length; index < len; index++) {
  //     numMissing -= this.bitfield.get(index)
  //   }
  //   return numMissing
  // }
  ...
  _getMetadataFromServer () {
    ...
    const tasks = urls.map((url) => {
      return (cb) => this._getMetadataFromURL(url, cb)
    })
    parallel(tasks)
  }

  _getMetadataFromURL (url, cb) {
    ...
    let req = null
    try {
      req = get.concat(opts, (err, res, torrent) => {
        this._handleMetadata(err, res, torrent, url, cb)
      })
    } catch (err) {
      this.emit('warning', new Error('skipping invalid url xs param: ' + url))
      return cb(null)
    }

    this._xsRequests.push(req)
  }

  _handleMetadata (err, res, torrent, url, cb) {
    if (this.destroyed) return cb(null)
    if (this.metadata) return cb(null)

    if (err) {
      this.emit('warning', new Error('http error from xs param: ' + url))
      return cb(null)
    }
    if (res.statusCode !== 200) {
      this.emit('warning', new Error('non-200 status code ' + res.statusCode + ' from xs param: ' + url))
      return cb(null)
    }
    ...
    if (parsedTorrent.infoHash !== this.infoHash) {
      this.emit('warning', new Error('got torrent file with incorrect info hash from xs param: ' + url))
      return cb(null)
    }

    this._onMetadata(parsedTorrent)
    cb(null)
  }
  ...
  _onMetadata (metadata) {
    ...
    if (this.client.enableWebSeeds) {
      this.urlList.forEach((url) => this.addWebSeed(url))
    }
    ...
    if (this._fileModtimes && this._store === FSChunkStore) {
      ...
      this.getFileModtimes((err, fileModtimes) => {
        ...
        const unchanged = this.files.map((_, index) => {
          return fileModtimes[index] === this._fileModtimes[index]
        }).every((x) => x)
  ...
  getFileModtimes (cb) {
    const ret = []
    parallelLimit(this.files.map((file, index) => {
      return (cb) => {
  ...
  _verifyPieces () {
    parallelLimit(this.pieces.map((_, index) => {
      return (cb) => {
  ...
  _destroy (err, cb) {
    ...
    const tasks = this._servers.map((server) => {
      return (cb) => server.destroy(cb)
    })

    if (this.discovery) {
      tasks.push((cb) => this.discovery.destroy(cb))
    }

    if (this.store) {
      tasks.push((cb) => this.store.close(cb))
    }
  ...
  _addPeer (peer) {
    if (this.destroyed) {
      this._debug('ignoring peer: torrent is destroyed')
      if (typeof peer !== 'string') peer.destroy()
      return null
    }
  ...
  _onWire (wire, addr) {
    ...
    if (this.client.dht && this.client.dht.listening) {
      wire.on('port', (port) => {
        ...
        this.client.dht.addNode({ host: wire.remoteAddress, port: port })
      })
    }
    ...
    if (typeof utPex === 'function' && !this.private) {
      ...
      // Don't sending updates to remote peer after the wire is closed
    ...
    if (this.metadata) {
      ...
      process.nextTick(() => this._onWireWithMetadata(wire))
    }
  }

  _onChokeTimeout (wire) {
    ...
    if (this._numQueued > 2 * (this._numConns - this.numPeers) &&
      ...
      this._setChokeTimeout(wire)
    }
  }

  _setChokeTimeout (wire) {
    wire._chokeTimeoutId = setTimeout(() => this._onChokeTimeout(wire), CHOKE_TIMEOUT)
    if (wire._chokeTimeoutId.unref) wire._chokeTimeoutId.unref()
  }

  _updateSeedStatus (wire) {
    if (wire.peerPieces.length !== this.pieces.length) return
    for (; wire._updatedPieces < this.pieces.length; ++wire._updatedPieces) {
      if (!wire.peerPieces.get(wire._updatedPieces)) return
    }
    wire.isSeeder = true
  ...
  _onWireWithMetadata (wire) {
    wire._chokeTimeoutId = null
    wire._updatedPieces = 0

    wire.on('bitfield', () => {
      this._updateSeedStatus(wire)
      this._update()
    })
    ...
    wire.once('close', () => clearTimeout(wire._chokeTimeoutId))

    wire.on('choke', () => {
      clearTimeout(wire._chokeTimeoutId)
      this._setChokeTimeout(wire)
    })
    ...
    if (wire.type !== 'webSeed') this._setChokeTimeout(wire)
  ...
  _gcSelections () {
    for (let i = 0; i < this._selections.length; i++) {
      ...
      this._selections.splice(i--, 1) // decrement i to offset splice
  ...
  _updateInterest () {
    ...
    this.wires.forEach((wire) => {
      ...
      if (this._amInterested) wire.interested()
  ...
  _validateWire (wire) {
    ...
    let i = this._selections.length
    while (i--) {
      const next = this._selections[i]
      let piece = null
      if (this.strategy === 'rarest') {
        const start = next.from + next.offset
        const end = next.to
        const len = end - start + 1
        const tried = {}
        let tries = 0
        const filter = this._genPieceFilterFunc(wire, start, end, tried)

        while (tries < len) {
          piece = this._rarityMap.getRarestPiece(filter)
          if (piece < 0) break
          if (this._request(wire, piece, false)) return
          tried[piece] = true
          tries += 1
        }
      } else {
  ...
  _speedRanker (wire) {
    ...
    return (index) => {
      if (!tries || this.bitfield.get(index)) return true

      let missing = this.pieces[index].missing

      for (; ptr < this.wires.length; ptr++) {
        const otherWire = this.wires[ptr]
  ...
  _shufflePriority (i) {
    let last = i
    for (let j = i; j < this._selections.length && this._selections[j].priority; j++) {
      last = j
    }
    const tmp = this._selections[i]
    this._selections[i] = this._selections[last]
    this._selections[last] = tmp
  }

  _trySelectWire (wire, hotswap) {
    const maxOutstandingRequests = getBlockPipelineLength(wire, PIPELINE_MAX_DURATION)
    if (wire.requests.length >= maxOutstandingRequests) return true
    const rank = this._speedRanker(wire)
    ...
    for (let i = 0; i < this._selections.length; i++) {
      ...
      if (this.strategy === 'rarest') {
        ...
        const tried = {}
        let tries = 0
        const filter = this._genPieceFilterFunc(wire, start, end, tried, rank)
        ...
        while (tries < len) {
          ...
          while (this._request(wire, piece, this._critical[piece] || hotswap)) {}
          ...
          if (next.priority) this._shufflePriority(i)
          return true
        }
      } else {
  ...
  _genPieceFilterFunc (wire, start, end, tried, rank) {
    return (i) => {
  ...
  _updateWire (wire) {
    ...
    if (!wire.downloaded) return this._validateWire(wire)

    const minOutstandingRequests = getBlockPipelineLength(wire, PIPELINE_MIN_DURATION)
    if (wire.requests.length >= minOutstandingRequests) return

    this._trySelectWire(wire, false) || this._trySelectWire(wire, true)
  }
  ...
  _hotswap (wire, index) {
    ...
    for (let i = 0; i < minWire.requests.length; i++) {
  ...
  _request (wire, index, hotswap) {
    ...
    // const endGame = (wire.requests.length === 0 && this.store.numMissing < 30)

    const piece = this.pieces[index]
    let reservation = isWebSeed ? piece.reserveRemaining() : piece.reserve()

    if (reservation === -1 && hotswap && this._hotswap(wire, index)) {
      reservation = isWebSeed ? piece.reserveRemaining() : piece.reserve()
    }
    if (reservation === -1) return false

    let r = this._reservations[index]
    if (!r) r = this._reservations[index] = []
    ...
    const onChunk = (err, chunk) => {
      ...
      if (piece !== this.pieces[index]) return this._onUpdateTick()
      ...
      if (!piece.set(reservation, chunk, wire)) return this._onUpdateTick()

      const buf = piece.flush()

      // TODO: might need to set this.pieces[index] = null here since sha1 is async
      ...
      sha1(buf, (hash) => {
        if (hash === this._hashes[index]) {
          ...
          this.store.put(index, buf)

          this.wires.forEach((wire) => wire.have(index))

          if (this._checkDone()) this.discovery.complete()
        } else {
          this.pieces[index] = new Piece(piece.length)
    ...
    wire.request(index, chunkOffset, chunkLength, (err, chunk) => {
      // TODO: what is this for?
      if (!this.ready) {
        return this.once('ready', () => onChunk(err, chunk))
      }
      onChunk(err, chunk)
    })
